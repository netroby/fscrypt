...
Once this is setup, you can run `make` to build the executable and PAM module in
the current directory (the components can be built separately with `make
fscrypt` or 'make pam_fscrypt'). See the `Makefile` for instructions on building
a static executable. The C libraries used by fscrypt will be dynamically linked
by default.
...
To install fscrypt and the pam_fscrypt module run `sudo make install` (the
components can be built separately with `sudo make install_fscrypt` or 'sudo
make install_pam').
...
The Password functionality of pam_fscrypt is used to automatically rewrap a
user's login protector when their unix passphrase changes. An easy way to get
...
The Auth and Session functionality of pam_fscrypt are used to automatically
